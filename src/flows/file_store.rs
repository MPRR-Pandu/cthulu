...
mod tests {
    ...
    async fn test_flow_list() {
        ...
        store.save_flow(test_flow("f1", "Flow 1")).await.unwrap();
        store.save_flow(test_flow("f2", "Flow 2")).await.unwrap();
    ...
    async fn test_flow_delete() {
        ...
        store.add_run(test_run("f1", "r1")).await.unwrap();

        let deleted = store.delete_flow("f1").await.unwrap();
        assert!(deleted);
    ...
    async fn test_run_persistence_across_restart() {
        ...
        store.add_run(test_run("f1", "r1")).await.unwrap();
        store.add_run(test_run("f1", "r2")).await.unwrap();
        drop(store);
        ...
        let runs = store2.get_runs("f1", 100).await;
    ...
    async fn test_run_cap_enforced() {
        ...
        for i in 0..105 {
            ...
            store.add_run(run).await.unwrap();
        }

        let runs = store.get_runs("f1", 200).await;
    ...
    async fn test_run_cap_enforced_on_load() {
        ...
        let runs = store2.get_runs("f1", 200).await;
        assert_eq!(runs.len(), MAX_RUNS_PER_FLOW);
    ...
    async fn test_complete_run() {
        ...
        let runs = store.get_runs("f1", 10).await;
        ...
        let store2 = FileStore::new(dir.path().to_path_buf());
        store2.load_all().await.unwrap();
        let runs = store2.get_runs("f1", 10).await;
        assert_eq!(runs[0].status, RunStatus::Success);
    }
    ...
    async fn test_push_node_run() {
        ...
        store.push_node_run("f1", "r1", nr).await.unwrap();
