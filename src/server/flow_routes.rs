...
pub fn flow_router() -> Router<AppState> {
    Router::new()
        ...
        .route("/flows/{id}/nodes/{node_id}/session", get(get_node_session))
        .route("/flows/{id}/nodes/{node_id}/interact", post(interact_node))
        .route("/flows/{id}/nodes/{node_id}/interact/sessions", get(list_node_sessions))
        .route("/flows/{id}/nodes/{node_id}/interact/new", post(new_node_session))
        .route("/flows/{id}/nodes/{node_id}/interact/sessions/{session_id}", delete(delete_node_session))
        .route("/flows/{id}/nodes/{node_id}/interact/stop", post(stop_node_interact))
        // Scheduler / cron endpoints
        .route("/flows/{id}/schedule", get(get_schedule))
...
// ---------------------------------------------------------------------------
// Node attachment endpoints (skill training files)
// ---------------------------------------------------------------------------
...
async fn list_node_sessions(
    ...
    let key = super::node_sessions_key(&id, &node_id);
    let sessions = state.interact_sessions.read().await;
...
async fn interact_node(
    ...
    let stream = async_stream::stream! {
        ...
        if needs_spawn {
            ...
            let live_proc = super::LiveClaudeProcess {
                stdin: child_stdin,
