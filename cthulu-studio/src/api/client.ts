...
export interface McpStatus {
  ...
  registered_in_claude_desktop: boolean;
  config_path: string;
  config_entry: Record<string, unknown> | null;
  searxng_ok: boolean;
  searxng_url: string;
}
...
export async function buildMcp(
  onLine: (line: string, isError: boolean, isDone: boolean) => void
): Promise<void> {
  const url = `${getBaseUrl()}/api/mcp/build`;
...
export async function registerMcp(options?: {
  baseUrl?: string;
  searxngUrl?: string;
}): Promise<{ ok: boolean; message?: string; error?: string; config_path?: string }> {
  return apiFetch("/mcp/register", {
    method: "POST",
    body: JSON.stringify({
      base_url: options?.baseUrl,
      searxng_url: options?.searxngUrl,
    }),
  });
}
